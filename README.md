...
| Браузер (YouTube и прочее) | значок браузера и заголовок вкладки | медиа-клавиши |
...
Заголовок вкладки браузера берётся через AppleScript. Отказ в доступе запоминается на две минуты: без этого запрещённый запрос повторялся бы каждые две секунды впустую.
...
Постоянная подпись сборки избавляет от повторных запросов: ad-hoc подпись меняется при каждой пересборке, и система переспрашивает разрешения заново.

```bash
SIGN_IDENTITY="Chelka Dev" make app
```
