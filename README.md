...
- [x] Управление музыкой: Music.app, Spotify и глобальные медиа-клавиши
...
Music.app и Spotify отдают название, исполнителя, обложку и позицию — виджет спрашивает их через AppleScript. Всё остальное (браузер, Яндекс.Музыка, VLC) управляется глобальными медиа-клавишами: кнопки работают, метаданных нет.

Так вышло не от лени: с macOS 15.4 системный Now Playing закрыт для сторонних приложений — приватный `MediaRemote` требует entitlement, который выдаётся только Apple. AppleScript остаётся единственным законным способом получить название трека.

Скрипты выполняются **отдельным процессом с таймаутом**. Подвисший Music.app подвесил бы `NSAppleScript` вместе с потоком, а внешний процесс просто снимается по таймауту — виджет продолжает работать. Закрытые плееры не опрашиваются вовсе: `tell application` запустил бы их сам.

Смена трека прилетает мгновенно через рассылку уведомлений, которую плееры шлют сами; опрос нужен только для позиции и идёт раз в две секунды, пока виджет раскрыт.
